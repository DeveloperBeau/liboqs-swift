...
/// MAYO-1 digital signatures (multivariate Oil-and-Vinegar scheme, NIST additional-signatures candidate).
///
...
/// let signer = try MAYO1.PrivateKey()
///
/// // Sign something
/// let sig = try signer.signature(for: messageData)
///
/// // Anyone with the public key can verify
/// let pub = try MAYO1.PublicKey(rawRepresentation: signerPublicKeyData)
...
/// let loaded = try MAYO1.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: signer.publicKey.rawRepresentation
/// )
/// ```
public enum MAYO1: Sendable {
    static let algorithmName = "MAYO-1"
    ...
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try sigGenerateKeyPair(algorithm: MAYO1.algorithmName)
            self.rawRepresentation = kp.secretKey
            self.publicKey = PublicKey(unchecked: kp.publicKey)
        }

        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try sigExpectedKeyLengths(algorithm: MAYO1.algorithmName)
        ...
        public func signature(for data: Data) throws -> Data {
            try sigSign(algorithm: MAYO1.algorithmName, message: data, secretKey: rawRepresentation)
        }
    }
    ...
    public struct PublicKey: Sendable {
        ...
        public func isValidSignature(_ signature: Data, for data: Data) throws -> Bool {
            try sigVerify(algorithm: MAYO1.algorithmName, message: data, signature: signature, publicKey: rawRepresentation)
        }
    }
}
