...
/// BIKE-L3 key encapsulation (NIST security category 3).
///
...
/// Round 4 KEM candidate. This variant, BIKE-L3, targets NIST security
/// category 3. The shared secret is 32 bytes.
///
/// ```swift
/// // Alice generates a key pair
/// let alice = try BIKEL3.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try BIKEL3.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try BIKEL3.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum BIKEL3: Sendable {
    static let algorithmName = "BIKE-L3"

    /// A BIKEL3 private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: BIKEL3.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: BIKEL3.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: BIKEL3.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A BIKEL3 public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: BIKEL3.algorithmName, publicKey: rawRepresentation)
