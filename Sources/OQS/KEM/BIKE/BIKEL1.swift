...
/// BIKE-L1 key encapsulation (NIST security category 1).
///
...
/// Round 4 KEM candidate. This variant, BIKE-L1, targets NIST security
/// category 1. The shared secret is 32 bytes.
///
/// ```swift
/// // Alice generates a key pair
/// let alice = try BIKEL1.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try BIKEL1.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try BIKEL1.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum BIKEL1: Sendable {
    static let algorithmName = "BIKE-L1"

    /// A BIKEL1 private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: BIKEL1.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: BIKEL1.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: BIKEL1.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A BIKEL1 public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: BIKEL1.algorithmName, publicKey: rawRepresentation)
