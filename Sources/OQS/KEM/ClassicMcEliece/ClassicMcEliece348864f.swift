...
/// Classic McEliece 348864f key encapsulation (conservative choice, studied for 50+ years, but with very large keys).
///
/// The smallest Classic McEliece parameter set, in its fast-keygen (`f`)
...
/// of the same size. Public keys are still huge (around 261 KB), but this
...
/// let alice = try ClassicMcEliece348864f.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try ClassicMcEliece348864f.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try ClassicMcEliece348864f.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum ClassicMcEliece348864f: Sendable {
    static let algorithmName = "Classic-McEliece-348864f"

    /// A ClassicMcEliece348864f private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: ClassicMcEliece348864f.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: ClassicMcEliece348864f.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: ClassicMcEliece348864f.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A ClassicMcEliece348864f public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: ClassicMcEliece348864f.algorithmName, publicKey: rawRepresentation)
