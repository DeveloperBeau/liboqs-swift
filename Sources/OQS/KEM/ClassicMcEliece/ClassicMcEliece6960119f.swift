...
/// Classic McEliece 6960119f key encapsulation (conservative choice, studied for 50+ years, but with very large keys).
///
...
/// let alice = try ClassicMcEliece6960119f.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try ClassicMcEliece6960119f.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try ClassicMcEliece6960119f.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum ClassicMcEliece6960119f: Sendable {
    static let algorithmName = "Classic-McEliece-6960119f"

    /// A ClassicMcEliece6960119f private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: ClassicMcEliece6960119f.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: ClassicMcEliece6960119f.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: ClassicMcEliece6960119f.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A ClassicMcEliece6960119f public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: ClassicMcEliece6960119f.algorithmName, publicKey: rawRepresentation)
