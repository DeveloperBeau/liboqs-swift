...
/// Classic McEliece 460896f key encapsulation (conservative choice, studied for 50+ years, but with very large keys).
///
/// A NIST level 3 Classic McEliece parameter set, in its fast-keygen (`f`)
...
/// of the same size. Public keys are enormous (around 524 KB), but this
...
/// let alice = try ClassicMcEliece460896f.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try ClassicMcEliece460896f.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try ClassicMcEliece460896f.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum ClassicMcEliece460896f: Sendable {
    static let algorithmName = "Classic-McEliece-460896f"

    /// A ClassicMcEliece460896f private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: ClassicMcEliece460896f.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: ClassicMcEliece460896f.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: ClassicMcEliece460896f.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A ClassicMcEliece460896f public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: ClassicMcEliece460896f.algorithmName, publicKey: rawRepresentation)
