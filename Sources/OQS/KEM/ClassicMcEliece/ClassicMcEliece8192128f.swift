...
/// Classic McEliece 8192128f key encapsulation (conservative choice, studied for 50+ years, but with very large keys).
///
...
/// of the same size. Public keys are enormous (around 1.3 MB), but this
...
/// let alice = try ClassicMcEliece8192128f.PrivateKey()
///
/// // Bob gets Alice's public key and creates a shared secret
/// let pub = try ClassicMcEliece8192128f.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try ClassicMcEliece8192128f.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum ClassicMcEliece8192128f: Sendable {
    static let algorithmName = "Classic-McEliece-8192128f"

    /// A ClassicMcEliece8192128f private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: ClassicMcEliece8192128f.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: ClassicMcEliece8192128f.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: ClassicMcEliece8192128f.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A ClassicMcEliece8192128f public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: ClassicMcEliece8192128f.algorithmName, publicKey: rawRepresentation)
