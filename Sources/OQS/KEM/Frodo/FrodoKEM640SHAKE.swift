...
/// FrodoKEM-640-SHAKE key encapsulation (NIST Round 3 alternate, ~128-bit security, SHAKE-based PRF).
///
...
/// let pub = try FrodoKEM640SHAKE.PublicKey(rawRepresentation: alicePublicKeyData)
...
/// let loaded = try FrodoKEM640SHAKE.PrivateKey(
///     rawRepresentation: saved,
///     publicKeyRepresentation: alice.publicKey.rawRepresentation
/// )
/// ```
public enum FrodoKEM640SHAKE: Sendable {
    static let algorithmName = "FrodoKEM-640-SHAKE"

    /// A FrodoKEM-640-SHAKE private (decapsulation) key.
    public struct PrivateKey: Sendable {
        ...
        public init() throws {
            let kp = try kemGenerateKeyPair(algorithm: FrodoKEM640SHAKE.algorithmName)
        ...
        public init(rawRepresentation: Data, publicKeyRepresentation: Data) throws {
            let lengths = try kemExpectedKeyLengths(algorithm: FrodoKEM640SHAKE.algorithmName)
        ...
        public func decryptSharedSecret(_ ciphertext: Data) throws -> SharedSecret {
            let ss = try kemDecapsulate(algorithm: FrodoKEM640SHAKE.algorithmName, ciphertext: ciphertext, secretKey: rawRepresentation)
            return SharedSecret(rawRepresentation: ss)
        }
    }

    /// A FrodoKEM-640-SHAKE public (encapsulation) key.
    public struct PublicKey: Sendable {
        ...
        public func generateSharedSecret() throws -> SharedSecretResult {
            let result = try kemEncapsulate(algorithm: FrodoKEM640SHAKE.algorithmName, publicKey: rawRepresentation)
